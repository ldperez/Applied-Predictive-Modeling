...
colnames(testCom)[17]<- "HeartDisease"

#Downsampling the data
trainFinal <- downSample(x =trainCom[,1:17], y= trainCom$HeartDisease, yname= "HeartDisease" )
dim(trainFinal)
```
...
```{r}
saveRDS(trainFinal, file="/Users/lamnguyen/Dropbox/Uyen bom/USD/ADS503/Final Project/Github/trainFinal.Rds")
saveRDS(testCom, file="/Users/lamnguyen/Dropbox/Uyen bom/USD/ADS503/Final Project/Github/testFinal.Rds")
```

# RESTART HERE

```{r}
trainFinal <- readRDS("/Users/lamnguyen/Dropbox/Uyen bom/USD/ADS503/Final Project/Github/trainFinal.Rds")
testCom <- readRDS("/Users/lamnguyen/Dropbox/Uyen bom/USD/ADS503/Final Project/Github/testFinal.Rds")
```
...
```{r}
set.seed(100)
ldaFit <- train(x =  trainFinal[, 1:25], 
                y = trainFinal$HeartDisease,
                method = "lda",
...
testResults$LDA <- predict(ldaFit, testCom[, 1:25])
```
...
set.seed(100)
glmnFit <- train(x = trainFinal[,1:25], 
                 y = trainFinal$HeartDisease,
                 method = "glmnet",
                 tuneGrid = glmnGrid,
                 preProc = c("center", "scale"),
                 ...
                 trControl = ctrl)
#glmnFit
testResults$glmnet <- predict(glmnFit, testCom[,1:25])
```
...
```{r}
testResults$Glmn <- predict(glmnFit, testCom[,1:25])
...
```{r}
set.seed(100)
nscFit <- train(x = trainFinal[, 1:25], 
                y = trainFinal$HeartDisease,
                method = "pam",
                preProc = c("center", "scale"),
...
```{r}
testResults$NSC <- predict(nscFit, testCom[, 1:25])
...
mdaFit <- train(x = trainFinal[, 1:25], 
                y = trainFinal$HeartDisease,
               method = "mda",
               preProc = c("center", "scale"),
...
```{r}
testResults$MDA <- predict(mdaFit, testCom[, 1:25])
...
nnetFit <- train(x = trainFinal[, 1:25], 
                y = trainFinal$HeartDisease,
                method = "nnet",
                metric = "ROC",
...
```{r}
testResults$nnet <- predict(nnetFit, testCom[,1:25])
...
```{r}
sigmaEst <- kernlab::sigest(as.matrix(trainFinal[, 1:25]))
svmgrid <- expand.grid(sigma = sigmaEst, C = 2^seq(-4,+4))

set.seed(100)
svmRFit <- train(x = trainFinal[,1:25], 
                y = trainFinal$HeartDisease,
                method = "svmRadial",
                tuneGrid = svmgrid,
                metric = "ROC",
...
```{r}
testResults$SVM <- predict(svmRFit, testCom[,1:25])
